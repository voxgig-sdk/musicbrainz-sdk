...
class InstrumentEntity
  ...
  def make
    opts = @_entopts.dup
    InstrumentEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Instrument.load works with no args).
  ...
  #
  # @param reqmatch [InstrumentListMatch, Hash, nil] match filter (any subset of
  #   Instrument fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
