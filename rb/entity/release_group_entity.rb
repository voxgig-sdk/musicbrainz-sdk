...
class ReleaseGroupEntity
  ...
  def make
    opts = @_entopts.dup
    ReleaseGroupEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.ReleaseGroup.load works with no args).
  ...
  #
  # @param reqmatch [ReleaseGroupListMatch, Hash, nil] match filter (any subset of
  #   ReleaseGroup fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
