...
class RecordingEntity
  ...
  def make
    opts = @_entopts.dup
    RecordingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MusicbrainzHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Recording, Hash] the current Recording data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MusicbrainzHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Recording fields)
  def match_get
  ...
  #
  # @param reqmatch [RecordingLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Recording.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Recording, Hash] the loaded Recording; raises MusicbrainzError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MusicbrainzHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [RecordingListMatch, Hash, nil] match filter (any subset of
  #   Recording fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Recording>, Array] the matching Recording items; raises MusicbrainzError on failure
