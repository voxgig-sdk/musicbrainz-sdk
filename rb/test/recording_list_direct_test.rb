...
class RecordingListDirectTest < Minitest::Test
  def test_direct_load_recording_list
    setup = recording_list_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-recording_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def recording_list_direct_setup(mockres)
