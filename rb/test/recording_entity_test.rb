...
class RecordingEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Recording(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Recording(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_RECORDING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def recording_basic_setup(extra)
  ...
  entid_env_raw = ENV["MUSICBRAINZ_TEST_RECORDING_ENTID"]
