...
class CollectionDirectTest < Minitest::Test
  def test_direct_list_collection
    setup = collection_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-collection", setup[:live] ? "live" : "unit")
    if _should_skip
...
def collection_direct_setup(mockres)
