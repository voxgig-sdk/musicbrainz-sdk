...
class EventEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Event(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Event(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_EVENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def event_basic_setup(extra)
  ...
  entid_env_raw = ENV["MUSICBRAINZ_TEST_EVENT_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["MUSICBRAINZ_TEST_EVENT_ENTID"])
