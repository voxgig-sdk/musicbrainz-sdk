...
class ReleaseEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Release(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Release(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_RELEASE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    release_ref01_data = nil
    if release_ref01_data_raw.length > 0
    ...
    release_ref01_list_result = release_ref01_ent.list(release_ref01_match, nil)
    assert release_ref01_list_result.is_a?(Array)
...
def release_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "release", "ReleaseTestData.json")
  ...
  entid_env_raw = ENV["MUSICBRAINZ_TEST_RELEASE_ENTID"]
