...
class WorkListEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = work_list_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "work_list." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_WORK_LIST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    work_list_ref01_data = nil
    if work_list_ref01_data_raw.length > 0
...
def work_list_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "work_list", "WorkListTestData.json")
  ...
  entid_env_raw = ENV["MUSICBRAINZ_TEST_WORK_LIST_ENTID"]
