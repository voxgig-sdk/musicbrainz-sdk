...
class WorkDirectTest < Minitest::Test
  def test_direct_list_work
    setup = work_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-work", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_work
    setup = work_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-work", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
