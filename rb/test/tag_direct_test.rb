...
class TagDirectTest < Minitest::Test
  def test_direct_load_tag
    setup = tag_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-tag", setup[:live] ? "live" : "unit")
    if _should_skip
