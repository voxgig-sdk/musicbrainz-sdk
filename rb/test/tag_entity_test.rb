...
class TagEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_TAG_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    tag_ref01_data_result, err = tag_ref01_ent.create(tag_ref01_data, nil)
    assert_nil err
...
def tag_basic_setup(extra)
  ...
  entid_env_raw = ENV["MUSICBRAINZ_TEST_TAG_ENTID"]
