# Musicbrainz SDK utility: make_request
...
module MusicbrainzUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MusicbrainzResponse.new({})
    ...
    if fetch_err
      ...
      response = MusicbrainzResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MusicbrainzResponse.new(fetched)
    else
