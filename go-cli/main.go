// Command Musicbrainz-cli is a CLI + REPL for the Musicbrainz SDK,
...
const prompt = "musicbrainz"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "area artist collection event genre instrument label place rating recording recording_list release release_group release_list series tag url work work_list"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MUSICBRAINZ_APIKEY (api key), MUSICBRAINZ_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MUSICBRAINZ_APIKEY carries the API key and
	// MUSICBRAINZ_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MUSICBRAINZ_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MUSICBRAINZ_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMusicbrainzSDK(opts)
