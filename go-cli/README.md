...
boru-driven command-line client **and** interactive REPL for the Musicbrainz
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/musicbrainz-cli)
make build

# 2. See usage (words, entities, env vars)
./musicbrainz-cli --help

# 3. Provide credentials once, via the environment
export MUSICBRAINZ_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./musicbrainz-cli list area
./musicbrainz-cli load 1 area            # {id:1} shorthand
./musicbrainz-cli load '{id:1}' area       # explicit match map
./musicbrainz-cli list artist

# 5. Override the API base URL for a single call
MUSICBRAINZ_BASE=https://api.example.com ./musicbrainz-cli list area

# 6. No arguments -> interactive REPL
./musicbrainz-cli
musicbrainz> list area
musicbrainz> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/musicbrainz-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/musicbrainz-cli list area
   ```
...
```sh
./musicbrainz-cli load 1 area          # scalar shorthand for {id:1}
./musicbrainz-cli load '{id:1}' area     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `musicbrainz>`). Each line is
...
below — this SDK exposes 19 entities.
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `area`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/musicbrainz-cli`. |
...
The 19 entities this SDK exposes (any is valid as `<entity>`):
...
(`./musicbrainz-cli <expr>`) and interactively (the REPL), and expressions compose the
