...
import (
	...
	omni "github.com/voxgig-sdk/musicbrainz-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MusicbrainzError); is {
				out := map[string]any{
...
func omniCtx(arg any, client *sdk.MusicbrainzSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MusicbrainzSDK); is {
			client = live
