...
func TestSeriesEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_SERIES_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func seriesBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MUSICBRAINZ_TEST_SERIES_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MUSICBRAINZ_TEST_SERIES_ENTID"])
