...
func TestUrlDirect(t *testing.T) {
	t.Run("direct-list-url", func(t *testing.T) {
		setup := urlDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-url", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-url", func(t *testing.T) {
		setup := urlDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-url", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type urlDirectSetupResult struct {
...
func urlDirectSetup(mockres any) *urlDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"MUSICBRAINZ_TEST_URL_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MUSICBRAINZ_TEST_URL_ENTID"]; ok {
		...
		return &urlDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &urlDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
