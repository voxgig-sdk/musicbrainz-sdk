...
func TestGenreDirect(t *testing.T) {
	t.Run("direct-list-genre", func(t *testing.T) {
		setup := genreDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-genre", _mode); _shouldSkip {
			if _reason == "" {
...
func genreDirectSetup(mockres any) *genreDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"MUSICBRAINZ_TEST_GENRE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MUSICBRAINZ_TEST_GENRE_ENTID"]; ok {
