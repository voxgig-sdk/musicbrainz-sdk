...
func TestPlaceEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_PLACE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		placeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.place")))
		...
		if placeRef01DataDt0LoadResult["id"] != placeRef01Data["id"] {
...
func placeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MUSICBRAINZ_TEST_PLACE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MUSICBRAINZ_TEST_PLACE_ENTID"])
