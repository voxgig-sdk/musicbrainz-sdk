...
func TestLabelEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_LABEL_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		labelRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.label", setup.data)))
		var labelRef01Data map[string]any
		if len(labelRef01DataRaw) > 0 {
			labelRef01Data = core.ToMapAny(labelRef01DataRaw[0][1])
		}
		...
		_, labelRef01ListOk := labelRef01ListResult.([]any)
		if !labelRef01ListOk {
...
func labelBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MUSICBRAINZ_TEST_LABEL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MUSICBRAINZ_TEST_LABEL_ENTID"])
