...
func TestSeriesDirect(t *testing.T) {
	t.Run("direct-list-series", func(t *testing.T) {
		setup := seriesDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-series", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-series", func(t *testing.T) {
		setup := seriesDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-series", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type seriesDirectSetupResult struct {
...
func seriesDirectSetup(mockres any) *seriesDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MUSICBRAINZ_TEST_SERIES_ENTID"]; ok {
		...
		return &seriesDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &seriesDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
