...
func TestUrlEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Url(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Url(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		urlRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.url", setup.data)))
		var urlRef01Data map[string]any
		if len(urlRef01DataRaw) > 0 {
			urlRef01Data = core.ToMapAny(urlRef01DataRaw[0][1])
		}
		...
		urlRef01Match := map[string]any{}

		urlRef01ListResult, err := urlRef01Ent.List(urlRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, urlRef01ListOk := urlRef01ListResult.([]any)
		if !urlRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", urlRef01ListResult)
		}

		// LOAD
		urlRef01MatchDt0 := map[string]any{
			"id": urlRef01Data["id"],
		}
		urlRef01DataDt0Loaded, err := urlRef01Ent.Load(urlRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		urlRef01DataDt0LoadResult := core.ToMapAny(entityData(urlRef01DataDt0Loaded))
		if urlRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if urlRef01DataDt0LoadResult["id"] != urlRef01Data["id"] {
...
func urlBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MUSICBRAINZ_TEST_URL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MUSICBRAINZ_TEST_URL_ENTID"])
