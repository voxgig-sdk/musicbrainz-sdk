...
func TestArtistDirect(t *testing.T) {
	t.Run("direct-list-artist", func(t *testing.T) {
		setup := artistDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-artist", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-artist", func(t *testing.T) {
		setup := artistDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-artist", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func artistDirectSetup(mockres any) *artistDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"MUSICBRAINZ_TEST_ARTIST_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MUSICBRAINZ_TEST_ARTIST_ENTID"]; ok {
		...
		return &artistDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &artistDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
