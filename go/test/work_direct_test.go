...
func TestWorkDirect(t *testing.T) {
	t.Run("direct-list-work", func(t *testing.T) {
		setup := workDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-work", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-work", func(t *testing.T) {
		setup := workDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-work", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type workDirectSetupResult struct {
...
func workDirectSetup(mockres any) *workDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["MUSICBRAINZ_TEST_WORK_ENTID"]; ok {
		...
		return &workDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &workDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
