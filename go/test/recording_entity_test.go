...
func TestRecordingEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Recording(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Recording(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_RECORDING_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		recordingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.recording")))
		...
		// LOAD
		recordingRef01MatchDt0 := map[string]any{
			"id": recordingRef01Data["id"],
		}
		recordingRef01DataDt0Loaded, err := recordingRef01Ent.Load(recordingRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		recordingRef01DataDt0LoadResult := core.ToMapAny(entityData(recordingRef01DataDt0Loaded))
		if recordingRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if recordingRef01DataDt0LoadResult["id"] != recordingRef01Data["id"] {
...
func recordingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MUSICBRAINZ_TEST_RECORDING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MUSICBRAINZ_TEST_RECORDING_ENTID"])
