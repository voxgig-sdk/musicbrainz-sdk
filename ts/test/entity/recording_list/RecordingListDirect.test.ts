...
describe('RecordingListDirect', async () => {
  ...
  test('direct-load-recording_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-recording_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["isrc01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MUSICBRAINZ_TEST_RECORDING_LIST_ENTID']
