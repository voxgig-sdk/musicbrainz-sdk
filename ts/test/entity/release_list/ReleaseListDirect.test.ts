...
describe('ReleaseListDirect', async () => {
  ...
  test('direct-load-release_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-release_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["discid01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MUSICBRAINZ_TEST_RELEASE_LIST_ENTID']
