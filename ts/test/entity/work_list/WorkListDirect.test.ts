...
describe('WorkListDirect', async () => {
  ...
  test('direct-load-work_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-work_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["iswc01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MUSICBRAINZ_TEST_WORK_LIST_ENTID']
