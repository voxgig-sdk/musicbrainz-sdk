...
(0, node_test_1.describe)('ReleaseGroupEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'release_group.' + op, live))
                return;
        }
        ...
        let release_group_ref01_data = Object.values(setup.data.existing.release_group)[0];
        // LIST
        const release_group_ref01_ent = client.ReleaseGroup();
        const release_group_ref01_match = {};
        const release_group_ref01_list = (await release_group_ref01_ent.list(release_group_ref01_match)).map((e) => e.data());
        // LOAD
        const release_group_ref01_match_dt0 = {};
        ...
        (0, node_assert_1.default)(release_group_ref01_data_dt0.id === release_group_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/release_group/ReleaseGroupTestData.json');
    ...
    let idmap = transform(['release_group01', 'release_group02', 'release_group03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['MUSICBRAINZ_TEST_RELEASE_GROUP_ENTID'];
...
//# sourceMappingURL=ReleaseGroupEntity.test.js.map