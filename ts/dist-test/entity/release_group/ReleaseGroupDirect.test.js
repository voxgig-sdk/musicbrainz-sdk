...
(0, node_test_1.describe)('ReleaseGroupDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.MusicbrainzSDK({
    ...
    (0, node_test_1.test)('direct-load-release_group', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-release_group', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-release_group', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-release_group', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.MUSICBRAINZ_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.MusicbrainzSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.MUSICBRAINZ_APIKEY,
        }));
        let idmap = env['MUSICBRAINZ_TEST_RELEASE_GROUP_ENTID'];
    ...
    const client = new __1.MusicbrainzSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ReleaseGroupDirect.test.js.map