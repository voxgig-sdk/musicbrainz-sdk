...
(0, node_test_1.describe)('ReleaseEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'release.' + op, live))
                return;
        }
        ...
        const release_ref01_match = {};
        const release_ref01_list = (await release_ref01_ent.list(release_ref01_match)).map((e) => e.data());
        // LOAD
...
function basicSetup(extra) {
    ...
    let idmap = transform(['release01', 'release02', 'release03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['MUSICBRAINZ_TEST_RELEASE_ENTID'];
