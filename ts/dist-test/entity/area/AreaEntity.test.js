...
(0, node_test_1.describe)('AreaEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list', 'load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'area.' + op, live))
                return;
        }
        ...
        let area_ref01_data = Object.values(setup.data.existing.area)[0];
        // LIST
        const area_ref01_ent = client.Area();
        const area_ref01_match = {};
        const area_ref01_list = (await area_ref01_ent.list(area_ref01_match)).map((e) => e.data());
        // LOAD
        const area_ref01_match_dt0 = {};
        area_ref01_match_dt0.id = area_ref01_data.id;
        const area_ref01_data_dt0 = (await area_ref01_ent.load(area_ref01_match_dt0)).data();
        (0, node_assert_1.default)(area_ref01_data_dt0.id === area_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/area/AreaTestData.json');
    ...
    let idmap = transform(['area01', 'area02', 'area03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['MUSICBRAINZ_TEST_AREA_ENTID'];
...
//# sourceMappingURL=AreaEntity.test.js.map