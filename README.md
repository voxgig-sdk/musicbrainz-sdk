...
The MusicBrainz API provides developers with access to the MusicBrainz Database, an extensive collection of music metadata.

Learn more about MusicBrainz API at [musicbrainz.org/doc/MusicBrainz_API](https://musicbrainz.org/doc/MusicBrainz_API).
...
// Load a specific recordinglist (returns a RecordingList)
...
// Load a specific recordinglist
recordingList, err := client.RecordingList(nil).Load(
    map[string]any{"isrc": "example_isrc"}, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(recordingList)
```
