...
class RecordingListEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RecordingListEntity(self._client, opts)
    ...
    def data_get(self) -> RecordingList:
    ...
    def match_get(self) -> RecordingList:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RecordingList:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RecordingList().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
