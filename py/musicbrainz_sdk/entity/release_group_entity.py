...
from musicbrainz_sdk.musicbrainz_types import (
    ReleaseGroup,
...
class ReleaseGroupEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ReleaseGroupEntity(self._client, opts)
    ...
    def data_get(self) -> ReleaseGroup:
    ...
    def match_get(self) -> ReleaseGroup:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ReleaseGroup:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ReleaseGroup().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ReleaseGroup]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ReleaseGroup().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
