...
class TestReleaseEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Release(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Release(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _release_basic_setup(None)
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MUSICBRAINZ_TEST_RELEASE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        release_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.release")))
        release_ref01_data = None
        if len(release_ref01_data_raw) > 0:
...
def _release_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/release/ReleaseTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MUSICBRAINZ_TEST_RELEASE_ENTID"))
