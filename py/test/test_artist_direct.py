...
class TestArtistDirect:

    def test_should_direct_list_artist(self):
        setup = _artist_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-artist", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_artist(self):
        setup = _artist_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-artist", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _artist_direct_setup(mockres):
