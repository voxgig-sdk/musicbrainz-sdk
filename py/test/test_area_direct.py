...
class TestAreaDirect:

    def test_should_direct_list_area(self):
        setup = _area_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-area", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_area(self):
        setup = _area_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-area", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _area_direct_setup(mockres):
