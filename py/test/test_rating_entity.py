    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MUSICBRAINZ_TEST_RATING_ENTID JSON to run live")
        client = setup["client"]
        ...
        rating_ref01_data = helpers.to_map(runner.entity_data(rating_ref01_ent.create(rating_ref01_data, None)))
...
def _rating_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MUSICBRAINZ_TEST_RATING_ENTID"))
