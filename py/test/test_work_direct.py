...
class TestWorkDirect:

    def test_should_direct_list_work(self):
        setup = _work_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-work", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_work(self):
        setup = _work_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-work", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _work_direct_setup(mockres):
