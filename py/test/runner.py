...
class MusicbrainzTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MusicbrainzTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MusicbrainzTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MusicbrainzTestRunner.getenv("MUSICBRAINZ_TEST_LIVE")
        override = MusicbrainzTestRunner.getenv("MUSICBRAINZ_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MusicbrainzTestRunner.getenv(key)
        ...
        explain = MusicbrainzTestRunner.getenv("MUSICBRAINZ_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MUSICBRAINZ_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MusicbrainzTestRunner._test_control is not None:
            return MusicbrainzTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MusicbrainzTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MusicbrainzTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MusicbrainzTestRunner.load_test_control()
...
def load_env_local():
    MusicbrainzTestRunner.load_env_local()


def env_override(m):
    return MusicbrainzTestRunner.env_override(m)


def entity_data(v):
    return MusicbrainzTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MusicbrainzTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MusicbrainzTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MusicbrainzTestRunner.load_test_control()


def live_client_options():
    return MusicbrainzTestRunner.live_client_options()


def live_delay_ms():
    return MusicbrainzTestRunner.live_delay_ms()
