...
class TestCollectionDirect:
    ...
    def test_should_direct_list_collection(self):
        setup = _collection_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-collection", "live" if setup["live"] else "unit")
        if _skip:
...
def _collection_direct_setup(mockres):
