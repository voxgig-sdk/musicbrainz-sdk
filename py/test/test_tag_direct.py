...
class TestTagDirect:
    ...
    def test_should_direct_load_tag(self):
        setup = _tag_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-tag", "live" if setup["live"] else "unit")
        if _skip:
