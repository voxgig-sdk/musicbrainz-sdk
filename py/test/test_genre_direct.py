...
class TestGenreDirect:
    ...
    def test_should_direct_list_genre(self):
        setup = _genre_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-genre", "live" if setup["live"] else "unit")
        if _skip:
