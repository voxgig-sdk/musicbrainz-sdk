...
class TestPlaceEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Place(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Place(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MUSICBRAINZ_TEST_PLACE_ENTID JSON to run live")
        client = setup["client"]
        ...
        place_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(place_ref01_data_dt0_loaded))
        assert place_ref01_data_dt0_load_result is not None
        assert place_ref01_data_dt0_load_result["id"] == place_ref01_data["id"]
...
def _place_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MUSICBRAINZ_TEST_PLACE_ENTID"))
