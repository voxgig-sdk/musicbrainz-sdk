...
class TestRecordingEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Recording(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Recording(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MUSICBRAINZ_TEST_RECORDING_ENTID JSON to run live")
        client = setup["client"]
        ...
        recording_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(recording_ref01_data_dt0_loaded))
...
def _recording_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MUSICBRAINZ_TEST_RECORDING_ENTID"))
