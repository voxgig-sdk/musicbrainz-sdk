...
type Args struct {
	Entity string         `json:"entity" jsonschema:"area | artist | collection | event | genre | instrument | label | place | rating | recording | recording_list | release | release_group | release_list | series | tag | url | work | work_list"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.MusicbrainzSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "musicbrainz_list",
		Description: "List records from Musicbrainz. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "musicbrainz_load",
		Description: "Load a single record from Musicbrainz. " +
...
func runOp(client *sdk.MusicbrainzSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.MusicbrainzSDK, name string) (sdk.MusicbrainzEntity, error) {
	...
	case "instrument":
		return client.Instrument(nil), nil
	case "label":
	...
	case "recording_list":
		return client.RecordingList(nil), nil
	case "release":
		return client.Release(nil), nil
	case "release_group":
		return client.ReleaseGroup(nil), nil
	case "release_list":
		return client.ReleaseList(nil), nil
	case "series":
	...
	case "work":
		return client.Work(nil), nil
	case "work_list":
		return client.WorkList(nil), nil
