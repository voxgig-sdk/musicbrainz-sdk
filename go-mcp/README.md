...
[MCP](https://modelcontextprotocol.io) server exposing the Musicbrainz SDK as
two agent tools — `musicbrainz_list` and `musicbrainz_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/musicbrainz-mcp)
make build

# 2. Provide credentials via the environment
export MUSICBRAINZ_APIKEY=sk_live_xxx
...
```jsonc
// musicbrainz_list: first page of records
{ "entity": "area" }
{ "entity": "area", "query": { } }

// musicbrainz_load: one record by id
{ "entity": "area", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/musicbrainz-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user musicbrainz \
     -- "$PWD"/dist/*/musicbrainz-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `musicbrainz_list` and `musicbrainz_load` tools now appear
   in new sessions. Ask the agent to *"list area using musicbrainz"*
   and it calls `musicbrainz_list` with `{"entity":"area"}`.
...
```sh
export MUSICBRAINZ_APIKEY=sk_live_xxx            # API key
export MUSICBRAINZ_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `musicbrainz_list` tool
...
### Call the `musicbrainz_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `musicbrainz_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `musicbrainz_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Field | Type | Notes |
|-------|------|-------|
| `entity` | string | One of the 19 supported entities (see below). |
...
| Variable | Purpose |
|----------|---------|
| `MUSICBRAINZ_APIKEY` | API key sent with every request. |
| `MUSICBRAINZ_BASE` | Optional override of the API base URL. |

### Entities

The 19 entities valid as the `entity` argument:
