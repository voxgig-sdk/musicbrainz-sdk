...
class TagEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_TAG_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$tag_ref01_data_result, $err] = $tag_ref01_ent->create($tag_ref01_data, null);
        $this->assertNull($err);
...
function tag_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MUSICBRAINZ_TEST_TAG_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MUSICBRAINZ_TEST_TAG_ENTID"]);
