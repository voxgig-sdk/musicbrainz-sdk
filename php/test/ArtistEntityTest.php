...
class ArtistEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Artist(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Artist(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = artist_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "artist." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_ARTIST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $artist_ref01_data = null;
        if (count($artist_ref01_data_raw) > 0) {
            $artist_ref01_data = Helpers::to_map($artist_ref01_data_raw[0][1]);
        }

        // LIST
        $artist_ref01_ent = $client->Artist(null);
        $artist_ref01_match = [];

        $artist_ref01_list_result = $artist_ref01_ent->list($artist_ref01_match, null);
        $this->assertIsArray($artist_ref01_list_result);
...
function artist_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/artist/ArtistTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["artist01", "artist02", "artist03"] as $k) {
    ...
    $entid_env_raw = getenv("MUSICBRAINZ_TEST_ARTIST_ENTID");
