...
class RecordingListEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MusicbrainzSDK::test(null, null);
        $ent = $testsdk->RecordingList(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = recording_list_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "recording_list." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_RECORDING_LIST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $recording_list_ref01_data = null;
        if (count($recording_list_ref01_data_raw) > 0) {
...
function recording_list_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/recording_list/RecordingListTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["recording_list01", "recording_list02", "recording_list03", "isrc01", "isrc02", "isrc03"] as $k) {
    ...
    $entid_env_raw = getenv("MUSICBRAINZ_TEST_RECORDING_LIST_ENTID");
