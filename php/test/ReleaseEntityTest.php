...
class ReleaseEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MusicbrainzSDK::test(null, null);
        $ent = $testsdk->Release(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Release(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Release(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = release_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "release." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_RELEASE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $release_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.release")));
        $release_ref01_data = null;
        if (count($release_ref01_data_raw) > 0) {
            $release_ref01_data = Helpers::to_map($release_ref01_data_raw[0][1]);
        }
        ...
        $release_ref01_list_result = $release_ref01_ent->list($release_ref01_match, null);
        $this->assertIsArray($release_ref01_list_result);
...
function release_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/release/ReleaseTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["release01", "release02", "release03"] as $k) {
    ...
    $entid_env_raw = getenv("MUSICBRAINZ_TEST_RELEASE_ENTID");
