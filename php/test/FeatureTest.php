...
class FtCtrl extends MusicbrainzControl
{
...
class FtClient
{
    ...
    private MusicbrainzUtility $utility;

    public function __construct(MusicbrainzUtility $utility, array $options)
    {
    ...
    public function get_utility(): MusicbrainzUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MusicbrainzContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MusicbrainzContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MusicbrainzContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MusicbrainzConfig::make_feature($name);
    ...
    private static function buildUrl(MusicbrainzSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MusicbrainzError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MusicbrainzResponse($fetched);
        }
    ...
    private function fail(MusicbrainzContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MusicbrainzContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
