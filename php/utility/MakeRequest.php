...
class MusicbrainzMakeRequest
{
    public static function call(MusicbrainzContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new MusicbrainzResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new MusicbrainzResponse($fetched);
        } else {
