...
class MusicbrainzMakeError
{
    public static function call(?MusicbrainzContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MusicbrainzContext([], null);
        }
        $op = $ctx->op ?? new MusicbrainzOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MusicbrainzResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MusicbrainzError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MusicbrainzError('', $msg, $ctx);
