...
describe("AreaDirect", function()
  it("should direct-list-area", function()
    local setup = area_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-area", setup.live and "live" or "unit")
  ...
  it("should direct-load-area", function()
    local setup = area_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-area", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function area_direct_setup(mockres)
