...
describe("InstrumentDirect", function()
  it("should direct-list-instrument", function()
    local setup = instrument_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-instrument", setup.live and "live" or "unit")
  ...
  it("should direct-load-instrument", function()
    local setup = instrument_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-instrument", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function instrument_direct_setup(mockres)
