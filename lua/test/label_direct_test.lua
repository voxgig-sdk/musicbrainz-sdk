...
describe("LabelDirect", function()
  it("should direct-list-label", function()
    local setup = label_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-label", setup.live and "live" or "unit")
  ...
  it("should direct-load-label", function()
    local setup = label_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-label", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function label_direct_setup(mockres)
