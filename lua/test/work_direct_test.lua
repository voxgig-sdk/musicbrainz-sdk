...
describe("WorkDirect", function()
  it("should direct-list-work", function()
    local setup = work_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-work", setup.live and "live" or "unit")
  ...
  it("should direct-load-work", function()
    local setup = work_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-work", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function work_direct_setup(mockres)
