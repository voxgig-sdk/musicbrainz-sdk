...
describe("UrlDirect", function()
  it("should direct-list-url", function()
    local setup = url_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-url", setup.live and "live" or "unit")
  ...
  it("should direct-load-url", function()
    local setup = url_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-url", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function url_direct_setup(mockres)
