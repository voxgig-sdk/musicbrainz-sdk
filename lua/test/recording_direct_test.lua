...
describe("RecordingDirect", function()
  it("should direct-list-recording", function()
    local setup = recording_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-recording", setup.live and "live" or "unit")
  ...
  it("should direct-load-recording", function()
    local setup = recording_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-recording", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function recording_direct_setup(mockres)
