...
describe("PlaceDirect", function()
  it("should direct-list-place", function()
    local setup = place_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-place", setup.live and "live" or "unit")
  ...
  it("should direct-load-place", function()
    local setup = place_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-place", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function place_direct_setup(mockres)
