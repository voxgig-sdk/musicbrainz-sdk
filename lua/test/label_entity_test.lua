...
describe("LabelEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Label(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Label(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_LABEL_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local label_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.label")))
    local label_ref01_data = nil
    if #label_ref01_data_raw > 0 then
      label_ref01_data = helpers.to_map(label_ref01_data_raw[1][2])
    end

    -- LIST
    local label_ref01_ent = client:Label(nil)
    local label_ref01_match = {}

    local label_ref01_list_result, err = label_ref01_ent:list(label_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(label_ref01_list_result)
...
function label_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MUSICBRAINZ_TEST_LABEL_ENTID")
