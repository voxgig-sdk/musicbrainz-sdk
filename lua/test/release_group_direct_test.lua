...
describe("ReleaseGroupDirect", function()
  it("should direct-list-release_group", function()
    local setup = release_group_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-release_group", setup.live and "live" or "unit")
  ...
  it("should direct-load-release_group", function()
    local setup = release_group_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-release_group", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function release_group_direct_setup(mockres)
