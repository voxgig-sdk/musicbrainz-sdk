...
describe("ArtistDirect", function()
  it("should direct-list-artist", function()
    local setup = artist_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-artist", setup.live and "live" or "unit")
  ...
  it("should direct-load-artist", function()
    local setup = artist_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-artist", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function artist_direct_setup(mockres)
