...
describe("RatingEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MUSICBRAINZ_TEST_RATING_ENTID JSON to run live")
      return
    end
    ...
    local rating_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LOAD
    local rating_ref01_match_dt0 = {}
    local rating_ref01_data_dt0_loaded, err = rating_ref01_ent:load(rating_ref01_match_dt0, nil)
    assert.is_nil(err)
    assert.is_not_nil(rating_ref01_data_dt0_loaded)
...
function rating_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MUSICBRAINZ_TEST_RATING_ENTID")
