...
describe("GenreDirect", function()
  it("should direct-list-genre", function()
    local setup = genre_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-genre", setup.live and "live" or "unit")
  ...
  it("should direct-load-genre", function()
    local setup = genre_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-genre", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function genre_direct_setup(mockres)
