...
describe("SeriesDirect", function()
  it("should direct-list-series", function()
    local setup = series_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-series", setup.live and "live" or "unit")
  ...
  it("should direct-load-series", function()
    local setup = series_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-series", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function series_direct_setup(mockres)
