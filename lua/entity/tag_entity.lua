...
local TagEntity = {}
TagEntity.__index = TagEntity


function TagEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TagEntity)
  self._name = "tag"
  self._client = client
...
function TagEntity:get_name()
  return self._name
end


function TagEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TagEntity.new(self._client, opts)
end
...
function TagEntity:mark_deleted()
  self._deleted = true
end


function TagEntity:deleted()
  return true == self._deleted
end


function TagEntity:data_set(args)
  if args ~= nil then
...
function TagEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TagEntity:match_set(args)
  if args ~= nil then
...
function TagEntity:stream(action, args, callopts)
...
---@param reqdata TagCreateData
---@param ctrl? table
---@return Tag
---@return string? err
function TagEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function TagEntity:_run_op(ctx, post_done)
  local utility = self._utility
