...
local WorkEntity = {}
WorkEntity.__index = WorkEntity


function WorkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorkEntity)
  self._name = "work"
  self._client = client
...
function WorkEntity:get_name()
  return self._name
end


function WorkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorkEntity.new(self._client, opts)
end
...
function WorkEntity:mark_deleted()
  self._deleted = true
end


function WorkEntity:deleted()
  return true == self._deleted
end


function WorkEntity:data_set(args)
  if args ~= nil then
...
function WorkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorkEntity:match_set(args)
  if args ~= nil then
...
function WorkEntity:stream(action, args, callopts)
...
function WorkEntity:_run_op(ctx, post_done)
  local utility = self._utility
