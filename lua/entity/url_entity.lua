...
local UrlEntity = {}
UrlEntity.__index = UrlEntity


function UrlEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UrlEntity)
  self._name = "url"
  self._client = client
...
function UrlEntity:get_name()
  return self._name
end


function UrlEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UrlEntity.new(self._client, opts)
end
...
function UrlEntity:mark_deleted()
  self._deleted = true
end


function UrlEntity:deleted()
  return true == self._deleted
end


function UrlEntity:data_set(args)
  if args ~= nil then
...
function UrlEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UrlEntity:match_set(args)
  if args ~= nil then
...
function UrlEntity:stream(action, args, callopts)
...
---@param reqmatch UrlLoadMatch
---@param ctrl? table
---@return Url
---@return string? err
function UrlEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch UrlListMatch
---@param ctrl? table
---@return Url[]
---@return string? err
function UrlEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function UrlEntity:_run_op(ctx, post_done)
  local utility = self._utility
