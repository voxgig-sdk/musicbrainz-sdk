...
local WorkListEntity = {}
WorkListEntity.__index = WorkListEntity


function WorkListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorkListEntity)
  self._name = "work_list"
  self._client = client
...
function WorkListEntity:get_name()
  return self._name
end


function WorkListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorkListEntity.new(self._client, opts)
end
...
function WorkListEntity:mark_deleted()
  self._deleted = true
end


function WorkListEntity:deleted()
  return true == self._deleted
end


function WorkListEntity:data_set(args)
  if args ~= nil then
...
function WorkListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorkListEntity:match_set(args)
  if args ~= nil then
...
function WorkListEntity:match_get()
...
function WorkListEntity:stream(action, args, callopts)
...
---@param reqmatch WorkListLoadMatch
---@param ctrl? table
---@return WorkList
---@return string? err
function WorkListEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WorkListEntity:_run_op(ctx, post_done)
  local utility = self._utility
