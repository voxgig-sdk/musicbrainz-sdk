...
local PlaceEntity = {}
PlaceEntity.__index = PlaceEntity


function PlaceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PlaceEntity)
  self._name = "place"
  self._client = client
...
function PlaceEntity:get_name()
  return self._name
end


function PlaceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PlaceEntity.new(self._client, opts)
end
...
function PlaceEntity:mark_deleted()
  self._deleted = true
end


function PlaceEntity:deleted()
  return true == self._deleted
end


function PlaceEntity:data_set(args)
  if args ~= nil then
...
function PlaceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PlaceEntity:match_set(args)
  if args ~= nil then
...
function PlaceEntity:match_get()
...
function PlaceEntity:stream(action, args, callopts)
...
---@param reqmatch PlaceLoadMatch
---@param ctrl? table
---@return Place
---@return string? err
function PlaceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PlaceListMatch
---@param ctrl? table
---@return Place[]
---@return string? err
function PlaceEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PlaceEntity:_run_op(ctx, post_done)
  local utility = self._utility
