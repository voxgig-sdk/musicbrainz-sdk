...
local ArtistEntity = {}
ArtistEntity.__index = ArtistEntity


function ArtistEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArtistEntity)
  self._name = "artist"
  self._client = client
...
function ArtistEntity:get_name()
  return self._name
end


function ArtistEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArtistEntity.new(self._client, opts)
end
...
function ArtistEntity:mark_deleted()
  self._deleted = true
end


function ArtistEntity:deleted()
  return true == self._deleted
end


function ArtistEntity:data_set(args)
  if args ~= nil then
...
function ArtistEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArtistEntity:match_set(args)
  if args ~= nil then
...
function ArtistEntity:match_get()
...
function ArtistEntity:stream(action, args, callopts)
...
function ArtistEntity:_run_op(ctx, post_done)
  local utility = self._utility
