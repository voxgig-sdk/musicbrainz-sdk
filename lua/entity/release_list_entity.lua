...
local ReleaseListEntity = {}
ReleaseListEntity.__index = ReleaseListEntity


function ReleaseListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReleaseListEntity)
  self._name = "release_list"
  self._client = client
...
function ReleaseListEntity:get_name()
  return self._name
end


function ReleaseListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReleaseListEntity.new(self._client, opts)
end
...
function ReleaseListEntity:mark_deleted()
  self._deleted = true
end


function ReleaseListEntity:deleted()
  return true == self._deleted
end


function ReleaseListEntity:data_set(args)
  if args ~= nil then
...
function ReleaseListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReleaseListEntity:match_set(args)
  if args ~= nil then
...
function ReleaseListEntity:match_get()
...
function ReleaseListEntity:stream(action, args, callopts)
...
---@param reqmatch ReleaseListLoadMatch
---@param ctrl? table
---@return ReleaseList
---@return string? err
function ReleaseListEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReleaseListEntity:_run_op(ctx, post_done)
  local utility = self._utility
