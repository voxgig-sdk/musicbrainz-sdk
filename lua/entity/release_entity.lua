...
local ReleaseEntity = {}
ReleaseEntity.__index = ReleaseEntity


function ReleaseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReleaseEntity)
  self._name = "release"
  self._client = client
...
function ReleaseEntity:get_name()
  return self._name
end


function ReleaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReleaseEntity.new(self._client, opts)
end
...
function ReleaseEntity:mark_deleted()
  self._deleted = true
end


function ReleaseEntity:deleted()
  return true == self._deleted
end


function ReleaseEntity:data_set(args)
  if args ~= nil then
...
function ReleaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReleaseEntity:match_set(args)
  if args ~= nil then
...
function ReleaseEntity:match_get()
...
function ReleaseEntity:stream(action, args, callopts)
...
function ReleaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
