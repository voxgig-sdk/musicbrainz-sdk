...
local RecordingEntity = {}
RecordingEntity.__index = RecordingEntity


function RecordingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RecordingEntity)
...
function RecordingEntity:get_name()
  return self._name
end


function RecordingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RecordingEntity.new(self._client, opts)
end
...
function RecordingEntity:mark_deleted()
  self._deleted = true
end


function RecordingEntity:deleted()
  return true == self._deleted
end


function RecordingEntity:data_set(args)
  if args ~= nil then
...
function RecordingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RecordingEntity:match_set(args)
  if args ~= nil then
...
function RecordingEntity:match_get()
...
function RecordingEntity:stream(action, args, callopts)
...
function RecordingEntity:_run_op(ctx, post_done)
  local utility = self._utility
