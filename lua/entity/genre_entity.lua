...
local GenreEntity = {}
GenreEntity.__index = GenreEntity


function GenreEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GenreEntity)
  self._name = "genre"
  self._client = client
...
function GenreEntity:get_name()
  return self._name
end


function GenreEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GenreEntity.new(self._client, opts)
end
...
function GenreEntity:mark_deleted()
  self._deleted = true
end


function GenreEntity:deleted()
  return true == self._deleted
end


function GenreEntity:data_set(args)
  if args ~= nil then
...
function GenreEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GenreEntity:match_set(args)
  if args ~= nil then
...
function GenreEntity:match_get()
...
function GenreEntity:stream(action, args, callopts)
...
---@param reqmatch GenreListMatch
---@param ctrl? table
---@return Genre[]
---@return string? err
function GenreEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function GenreEntity:_run_op(ctx, post_done)
  local utility = self._utility
