...
local LabelEntity = {}
LabelEntity.__index = LabelEntity


function LabelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LabelEntity)
  self._name = "label"
  self._client = client
...
function LabelEntity:get_name()
  return self._name
end


function LabelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LabelEntity.new(self._client, opts)
end
...
function LabelEntity:mark_deleted()
  self._deleted = true
end


function LabelEntity:deleted()
  return true == self._deleted
end


function LabelEntity:data_set(args)
  if args ~= nil then
...
function LabelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LabelEntity:match_set(args)
  if args ~= nil then
...
function LabelEntity:match_get()
...
function LabelEntity:stream(action, args, callopts)
...
---@param reqmatch LabelLoadMatch
---@param ctrl? table
---@return Label
---@return string? err
function LabelEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch LabelListMatch
---@param ctrl? table
---@return Label[]
---@return string? err
function LabelEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function LabelEntity:_run_op(ctx, post_done)
  local utility = self._utility
