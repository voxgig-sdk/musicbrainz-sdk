...
local AreaEntity = {}
AreaEntity.__index = AreaEntity


function AreaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AreaEntity)
  self._name = "area"
  self._client = client
...
function AreaEntity:get_name()
  return self._name
end


function AreaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AreaEntity.new(self._client, opts)
end
...
function AreaEntity:mark_deleted()
  self._deleted = true
end


function AreaEntity:deleted()
  return true == self._deleted
end


function AreaEntity:data_set(args)
  if args ~= nil then
...
function AreaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AreaEntity:match_set(args)
  if args ~= nil then
...
function AreaEntity:stream(action, args, callopts)
...
---@param reqmatch AreaListMatch
---@param ctrl? table
---@return Area[]
---@return string? err
function AreaEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function AreaEntity:_run_op(ctx, post_done)
  local utility = self._utility
