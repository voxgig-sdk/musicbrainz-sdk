...
local InstrumentEntity = {}
InstrumentEntity.__index = InstrumentEntity


function InstrumentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InstrumentEntity)
...
function InstrumentEntity:get_name()
  return self._name
end


function InstrumentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InstrumentEntity.new(self._client, opts)
end
...
function InstrumentEntity:mark_deleted()
  self._deleted = true
end


function InstrumentEntity:deleted()
  return true == self._deleted
end


function InstrumentEntity:data_set(args)
  if args ~= nil then
...
function InstrumentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InstrumentEntity:match_set(args)
  if args ~= nil then
...
function InstrumentEntity:match_get()
...
function InstrumentEntity:stream(action, args, callopts)
...
function InstrumentEntity:_run_op(ctx, post_done)
  local utility = self._utility
