...
local SeriesEntity = {}
SeriesEntity.__index = SeriesEntity


function SeriesEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SeriesEntity)
  self._name = "series"
  self._client = client
...
function SeriesEntity:get_name()
  return self._name
end


function SeriesEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SeriesEntity.new(self._client, opts)
end
...
function SeriesEntity:mark_deleted()
  self._deleted = true
end


function SeriesEntity:deleted()
  return true == self._deleted
end


function SeriesEntity:data_set(args)
  if args ~= nil then
...
function SeriesEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SeriesEntity:match_set(args)
  if args ~= nil then
...
function SeriesEntity:match_get()
...
function SeriesEntity:stream(action, args, callopts)
...
function SeriesEntity:_run_op(ctx, post_done)
  local utility = self._utility
