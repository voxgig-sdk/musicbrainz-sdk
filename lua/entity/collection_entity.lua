...
CollectionEntity.__index = CollectionEntity


function CollectionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CollectionEntity)
...
function CollectionEntity:get_name()
  return self._name
end


function CollectionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CollectionEntity.new(self._client, opts)
end
...
function CollectionEntity:mark_deleted()
  self._deleted = true
end


function CollectionEntity:deleted()
  return true == self._deleted
end


function CollectionEntity:data_set(args)
  if args ~= nil then
...
function CollectionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CollectionEntity:match_set(args)
  if args ~= nil then
...
function CollectionEntity:match_get()
...
function CollectionEntity:stream(action, args, callopts)
...
function CollectionEntity:_run_op(ctx, post_done)
  local utility = self._utility
