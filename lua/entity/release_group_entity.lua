...
local ReleaseGroupEntity = {}
ReleaseGroupEntity.__index = ReleaseGroupEntity


function ReleaseGroupEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReleaseGroupEntity)
  self._name = "release_group"
  self._client = client
...
function ReleaseGroupEntity:get_name()
  return self._name
end


function ReleaseGroupEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReleaseGroupEntity.new(self._client, opts)
end
...
function ReleaseGroupEntity:mark_deleted()
  self._deleted = true
end


function ReleaseGroupEntity:deleted()
  return true == self._deleted
end


function ReleaseGroupEntity:data_set(args)
  if args ~= nil then
...
function ReleaseGroupEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReleaseGroupEntity:match_set(args)
  if args ~= nil then
...
function ReleaseGroupEntity:match_get()
...
function ReleaseGroupEntity:stream(action, args, callopts)
...
---@param reqmatch ReleaseGroupLoadMatch
---@param ctrl? table
---@return ReleaseGroup
---@return string? err
function ReleaseGroupEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ReleaseGroupListMatch
---@param ctrl? table
---@return ReleaseGroup[]
---@return string? err
function ReleaseGroupEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ReleaseGroupEntity:_run_op(ctx, post_done)
  local utility = self._utility
