...
The Lua SDK for the Musicbrainz API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Area(nil):list(nil, nil)
...
```lua
local result, err = client:Area(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:Musicbrainz(nil):load(
  { id = "test01" }, nil
)
